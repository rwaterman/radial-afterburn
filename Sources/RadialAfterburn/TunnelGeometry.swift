...
enum TunnelGeometry {
    ...
    static func angle(lane: Int) -> Float {
        angle(laneFraction: Float(lane))
    }

    /// Continuous angle for a fractional lane, so a sprite can sit between lanes mid-slide.
    static func angle(laneFraction: Float) -> Float {
        laneFraction / Float(laneCount) * .pi * 2 - .pi / 2
    }
    ...
    static func worldPoint(lane: Int, depth: Float, time: Float = 0, wave: Int = 0, kick: Float = 0) -> SIMD3<Float> {
        worldPoint(laneFraction: Float(lane), depth: depth, time: time, wave: wave, kick: kick)
    }

    /// Same as `worldPoint(lane:…)` but for a continuous lane position (smooth slides).
    static func worldPoint(laneFraction: Float, depth: Float, time: Float = 0, wave: Int = 0, kick: Float = 0) -> SIMD3<Float> {
        let a = angle(laneFraction: laneFraction)
        let nearField = 1 - depth
        let warp = sin(time * 7.5 + depth * 18 + laneFraction * 0.7) * (0.05 * kick * nearField)
        let r = radius + warp
